...
static DWORD find_free_txurb(candle_device_t *dev, uint32_t timeout)
{
	...
	if (wait_result == WAIT_TIMEOUT) {
		...
		if (wait_result == WAIT_TIMEOUT) {
			dev->last_error = CANDLE_ERR_WRITE_TIMEOUT_PRE;
			dev->windows_last_error = GetLastError();
			return 0xffffffff;
		}
	}
...
DLL void __stdcall candle_windows_error_text(DWORD errnum, wchar_t* buffer, size_t buflen)
{
	FormatMessageW(FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_IGNORE_INSERTS,
		...
		buffer, (buflen / sizeof(wchar_t)), NULL);
}
#endif
